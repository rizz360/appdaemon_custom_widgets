...
class VacuumControl(hass.Hass):
    ...
    def get_vacuums_callback(self, args: Dict, request: Dict) -> Tuple[Dict, int]:
        ...
        try:
            ...
            self.log(
                f"Available vacuums: {[v['id'] for v in vacuums]}"
            )  # Log the IDs of available vacuums
    ...
    def get_rooms_callback(self, args: Dict, request: Dict) -> Tuple[Dict, int]:
        ...
        try:
            ...
            if not state:
                return {"success": False, "error": f"Vacuum {entity_id} not found"}, 404
            ...
            room_data = list(rooms_map.values())[
                -1
            ]  # Get the last map, you can adjust this if you want another strategy
            ...
            for room in room_data:
                try:
                    ...
                    self.log(
                        f"Error processing room data: {room}, Error: {str(e)}",
                        level="WARNING",
                    )
    ...
    def start_cleaning_callback(self, args: dict, request) -> tuple:
        ...
        try:
            ...
            if not segments or not isinstance(segments, list):
                ...
                return {
                    "success": False,
                    "error": "Invalid segments: must be a non-empty list!",
                }, 400
            ...
            # Start cleaning
            self.log(
                f"Starting cleaning for {entity_id}, segments: {segments}, repeats: {repeats}"
            )
            self.call_service(
